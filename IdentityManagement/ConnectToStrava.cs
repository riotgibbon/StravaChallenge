...
namespace IdentityManagement
{
    public class ConnectToStrava
    {
        ...
        private string host;
        private StravaDetails details;

        public ConnectToStrava(string host)
        {
            httpClient = new HttpClient();
            this._host = host;
        }

        public ConnectToStrava(HttpClient httpClient, string host)
        {
        ...
        public ConnectToStrava(HttpClient httpClient, StravaDetails details)
        {
            // TODO: Complete member initialization
            this.httpClient = httpClient;
        ...
        public ConnectToStrava(StravaDetails details): this(details.Host)
        {
            this.details = details;
        }
        

        public async Task<StravaAccessTokenInfo> GetTokenInfo(string accessToken)
        {
            var userResponse = await GetJson(HttpMethod.Get, TokenInfoEndpoint + "?access_token=" + Uri.EscapeDataString(accessToken));
            var StravaAccessTokenInfo = JsonConvert.DeserializeObject<StravaAccessTokenInfo>(userResponse);
            return StravaAccessTokenInfo;
        }
        ...
        public async Task<StravaAccessToken> SwapCodeForToken(string code, string redirectUri)
        {
            var body = GetTokenRequestBody(code, redirectUri);
            return await GetStravaAccessToken(body);
        }

        public async Task<StravaAccessToken> GetOAuthToken(string username, string password, string scope)
        {
        ...
        private async Task<StravaAccessToken> GetStravaAccessToken(IEnumerable<KeyValuePair<string, string>> body)
        {
            var tokenJson = await GetJson(HttpMethod.Post, TokenEndpoint, body);
            var token = JsonConvert.DeserializeObject<StravaAccessToken>(tokenJson);
            return token;
        }


        public async Task<StravaAccessToken> RefreshToken(string refreshToken, string scope)
        {
